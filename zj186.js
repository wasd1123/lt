...
!(async () => {
  if (typeof $request !== 'undefined' && $request.url.includes('wechat.zj186.com')) {
    $.debug($.toStr($request.headers, {}, null, 2))
    $.debug($.toStr($request.body, {}, null, 2))
    ...
    await notify(TITLE, '获取到请求信息', '已保存', {
      $media: 'https://raw.githubusercontent.com/Koolson/Qure/master/IconSet/China_Unicom.png',
    })
  } else {
    const request = $.getjson(`${NAMESPACE}`)
    if (!request) throw new Error('请先获取请求信息')
    ...
    if (code != 0) throw new Error(`签到失败: ${$.lodash_get(body, 'msg') || '未知错误'}`)
    const amount = $.lodash_get(body, 'result.0.amount')
    await notify(TITLE, `签到成功`, `获得沃币: ${amount || '-'}`, {
      $media: 'https://raw.githubusercontent.com/Koolson/Qure/master/IconSet/China_Unicom.png',
    })
  }
})()
